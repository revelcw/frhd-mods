...
(function() {
  ...
  const getUserActivity = async (user) => {
    ...
    if (userData?.friends.friends_data.length > 0) {
      ...
      return friendData.friends.friends_data.find((friend) => friend.u_name.toLowerCase() === user.toLowerCase()).activity_time_ago
    } else {
  ...
  const main = async () => {
    let timer = setInterval(async () => {
      if (document.querySelector("#content > div > div.profile-header > div.profile-user-info.clearfix > div > div.profile-info > div:nth-child(2)")) {
        clearInterval(timer)
        const url = new URL(document.URL)
        const user = url.pathname.split("/")[2]
        const lastPlayed = await getUserActivity(user)
        if (lastPlayed) {
          const parent = document.querySelector("#content > div > div.profile-header > div.profile-user-info.clearfix > div > div.profile-info > div:nth-child(2)")
          const text = document.createElement("span")
          text.classList.add("profile-blurb");
          text.style = 'position: absolute;padding: 2px;padding-left: 5px;'
